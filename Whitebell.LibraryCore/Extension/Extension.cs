...
namespace Whitebell.Library.Extension
{
    ...
    public static class Extension
    {
        ...
        public static bool InRange<T>(this T val, T min, T max, Interval interval) where T : IComparable<T>
        {
            ...
            {
                Interval.Closed => val.CompareTo(min) >= 0 && val.CompareTo(max) <= 0,
                Interval.Open => val.CompareTo(min) > 0 && val.CompareTo(max) < 0,
                Interval.LeftClosedRightOpen => val.CompareTo(min) >= 0 && val.CompareTo(max) < 0,
                Interval.LeftOpenRightClosed => val.CompareTo(min) > 0 && val.CompareTo(max) <= 0,
                _ => throw new ArgumentException(nameof(interval)),
            };
        }
        ...
        [SuppressMessage("スタイル", "IDE0047:不要なかっこの削除", Justification = "<保留中>")]
        ...
        [SuppressMessage("Redundancy", "RCS1215:Expression is always equal to true/false.", Justification = "<保留中>")]
        [SuppressMessage("スタイル", "IDE0047:不要なかっこの削除", Justification = "<保留中>")]
